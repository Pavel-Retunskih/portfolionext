  ...
  return (<div className={'flex flex-col lg:flex-row lg:gap-10 md:ml-[300px] border-t border-t-lines'}>
    ...
    <form className={'flex flex-col gap-8 border-r  border-r-lines px-32 pt-28'}>
      <div className={'flex flex-col gap-1'}>
        ...
        <input type="text" id={'name'} name={'name'} className={'w-[327px] border border-lines bg-primary-blue'}
               value={name} onChange={(e) => setName(e.target.value)}/>
      </div>
      <div className={'flex flex-col gap-1'}>
        <label htmlFor={'email'} className={'text-secondary-grey'}>_email:</label>
        <input type="text" id={'email'} name={'email'} className={'w-[327px] border border-lines bg-primary-blue'}
               value={email} onChange={(e) => setEmail(e.target.value)}/>
      </div>
      <div className={'flex flex-col gap-1'}>
        <label htmlFor={'message'} className={'text-secondary-grey'}>_message:</label>
        <textarea name={'message'} className={'w-[327px] h-[145px] border border-lines bg-primary-blue'} value={message}
